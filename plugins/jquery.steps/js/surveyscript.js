...
!function (a, b) {
    ...
    function btn_next(d, f, g) {
        var index_step = g.currentIndex;
        var check_next_step = 0;
        var base_url_js = jQuery("#base_url_js").val();
        var get_current_div_id = jQuery("#wizard-vertical-p-" + index_step).children("div").attr('id');
        var get_all_data = jQuery('#' + get_current_div_id + ' input[type=\'text\'], #' + get_current_div_id + ' input[type=\'hidden\'], #' + get_current_div_id + ' input[type=\'radio\']:checked, #' + get_current_div_id + ' input[type=\'checkbox\']:checked, #' + get_current_div_id + ' select, #' + get_current_div_id + ' textarea');
        var read_c_r_box = jQuery('#' + get_current_div_id + ' input[type=\'radio\']');

        var field_validation = 0;
        jQuery.each(get_all_data, function (key, value) {
          if (jQuery(this).attr("field") == "required") {
        ...
        var notcheckedvalue = [];
          ...
          jQuery.each(read_c_r_box, function (key1, value1) {
             ...
             if (jQuery(this).prop('checked') == false && jQuery(this).attr("field") == "required") {
          ...
          if (notcheckedvalue.length !== 0) {
              ...
              alert("All the values mentioned in question as mandatory");
              field_validation = 1;
          }

           if (index_step == 3) {
            jQuery("#comment10").removeClass('parsley-error');
            jQuery("#comment11").removeClass('parsley-error');
            if(jQuery("#special_recognition").val()==1 && jQuery("#comment10").val()==''){
              jQuery("#comment10").addClass('parsley-error');
              field_validation = 1;
            }
            if(jQuery("#issue_at_work").val()==1 && jQuery("#comment11").val()==''){
              jQuery("#comment11").addClass('parsley-error');
              field_validation = 1;
            }
          }
          if(index_step == 4){
            jQuery("#comment15").removeClass('parsley-error');
            if(jQuery("#come_back").val()==1 && jQuery("#comment15").val()==''){
              jQuery("#comment15").addClass('parsley-error');
              field_validation = 1;
            }
          }
          ...
          if (field_validation == 1) {
            ...
            if(index_step == 4){
              jQuery("a[href='#finish']").css('pointer-events', 'none');
              jQuery("a[href='#previous']").css('pointer-events', 'none');
            }
            check_next_step = 1;
          }
    ...
    function C(b) {
        ...
        switch (i.substring(i.lastIndexOf("#") + 1)) {
            case "cancel":
                ...
                break;
            case "finish":
                var revalue = btn_next(d, f, g);
                if (revalue == 0) {
                    return false;
                }
                // else if (revalue == 2) {
                //     var base_url_js = jQuery("#base_url_js").val();
                //     window.location.href = base_url_js + "CandidateRegister/thankspage?success=1";
                //     return false;
                // }
                else {
                    h(d, g);
