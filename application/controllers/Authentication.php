...
class Authentication extends REST_Controller {
    ...
    public function registration_post() {
        ...
        $password = $this->post('password');
        $phone = strip_tags($this->post('mobile_number'));
      ...
      if(!empty($name) && !empty($email) && !empty($password)){
            ...
            $userdataCount = $this->user->getRows($con);
            if($userdataCount > 0){
                // Set the response and exit
    ...
    public function user_put() {
        ...
        $user_name = strip_tags($this->put('user_name'));
        $email = strip_tags($this->put('email'));
        $password = $this->put('password');
        $phone = strip_tags($this->put('mobile_number'));

        // Validate the post data
        if(!empty($id) && (!empty($name) || !empty($user_name) || !empty($email) || !empty($password) || !empty($phone))){
